'''
classifies the vibration and runs an anomaly detection on the vibration data
from the connected acceleromter

need to have an .fbz akida model and .eim anomaly detection file on disk
'''
...
#for running anomaly detection eim
...
def help():
    print('python3 class-hw-timed-anom.py <path-to-akida-model.fbz> <path-to-anomaly-score.fbz')

def main(argv):
    parser = argparse.ArgumentParser(description='acquire accel data and run inference on akida')
    parser.add_argument('--fbz_file', type=str, required=True,
                        help='The path to the fbz file')
    parser.add_argument('--anomaly_eim', type=str, required=True,
                        help='The path to the anomaly scoring eim file')

    args = parser.parse_args()

    modelfile = args.fbz_file
    anom_score_eim = args.anomaly_eim
    st = os.stat(anom_score_eim)
    os.chmod(anom_score_eim, st.st_mode | stat.S_IEXEC)
    ...
    #run the dsp, channe raw accelerometer data into features for the model
    processed_features = dsp(features)

    #run the model on akida to classify the accelerometer data
    ...
    print("center: ", softmaxed_pred[0][0][0][0])
    print("edge: ", softmaxed_pred[0][0][0][1])
    print("off: ", softmaxed_pred[0][0][0][2])
    print("on: ", softmaxed_pred[0][0][0][3])

    #anomaly detection
    print(os.getcwd())
    runner = ImpulseRunner(os.path.abspath(anom_score_eim))
    try:
